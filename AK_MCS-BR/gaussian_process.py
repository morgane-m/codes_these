...
#        else:
    ...
    def K_eig_noncond(self, eps, Omega,n_start=11):
        """
        This function computes the eigenvalues and the eigenvectors of B=sqrt(W) * C * sqrt(W)
        ...
        nodes : nodes of the uniform grid 
        weights
        -> ordered for decreasing eigenvalues
        ...
        for it in range(1):
            ...
            # Simpson rule : uniform grid
            n_int=n_start+1
#            if self.normalize == True:
#                nodes = (nodes - self.X_mean) / self.X_std
#                Omega = (Omega - self.X_mean.reshape(Omega.shape[0],1)) / self.X_std.reshape(Omega.shape[0],1)
            h = (Omega[:,1] - Omega[:,0]) /float(n_int)
            h=float(h.prod())

            weights = np.zeros(nodes.shape[0])
            for i in np.arange(1,len(weights) / 2):
...
#            plt.plot(nodes,vect_star[:,0])
    ...
    def K_eig_noncond_EOLE(self, nb_vect, Omega,n_start=2000,eps = 1e-8):
        """
        This function computes the eigenvalues and the eigenvectors of B=sqrt(W) * C * sqrt(W)
        C : the non conditioned co variance matrix for the PCA
        Parameters
        ...
        for it in range(1):
            ...
            while n_comp <= nb_vect and crit_f == False : 
                vect_star = svd.components_.T
                eig=eig_new
                n_comp=n_comp+50
                svd = TruncatedSVD(n_components=n_comp, n_iter=10, random_state=1)
...
#            plt.plot(nodes,vect_star[:,0])
    ...
    def K_eig_noncond_mc(self, eps, distrib,n_start=1000,kde=False):
        """
        This function computes the eigenvalues and the eigenvectors of B=sqrt(W) * C * sqrt(W)
        C : the non conditioned co variance matrix for the PCA
        Parameters
        ...
        """
        if kde :
            nodes = distrib.resample(n_start).T
        else: 
            nodes = np.array(distrib.getSample(n_start))
        if self.normalize == True:
                nodes = (nodes - self.X_mean) / self.X_std
 
        weights =  np.ones(nodes.shape[0])
        weights = weights/ n_start
        W = np.zeros((nodes.shape[0],nodes.shape[0]))
        np.fill_diagonal(W, weights)
    ...
    def eval_eigenfunction_noncond(self, weights, nodes, eig, vect, X_new, M):
        ...
        vect : array_like
            eigenvectors
                M : int
            trunc number
        X_new : n x n_mc array
            n : coord nb of points 
            n_mc : nb of new points 
        Returns
        -------
...
#
    ...
    def eval_KL_expansion_trajectories(self, weights, nodes, eig, vect, M, x_new,xi_traj):
        """
        This function computes the eigenfunctions from the result
        of the Nystrom method.
    ...
    def eval_noncond_trajectories(self, x_new,n_traj):
        """
        ...
        n_targets=self.y.shape[1]
        n_mc,n=x_new.shape
        nodes=np.concatenate((self.X,x_new))
        H_x=np.zeros((n_traj,n_mc))
        nugget=1e-12
        # non conditioned  co variance matrix construction at nodes
        D_new, ij = l1_cross_distances(nodes)
        ...
        R_new = np.eye(nodes.shape[0]) * (1. + nugget)
        R_new[ij[:, 0], ij[:, 1]] = r_new
        R_new[ij[:, 1], ij[:, 0]] = r_new
        K=self.sigma2*R_new
        ## trajectories of GP_noncond
            # Cholesky factorization of K
        C_n = np.sqrt(self.sigma2) * linalg.cholesky(K, lower=True)
        # Mean prediction
        mean = np.zeros((n_doe+n_mc,1))
        mean = np.atleast_2d(mean).T
        # Trajectories
        H_tot = []
        for i in range(n):
            xi = np.random.normal(0.0, 1.0,( n_doe+n_mc,n_traj))
            H_tot.append(mean[:, 0] + np.dot(C_n, xi))
        H_tot = np.array(H_tot)[0,:,:].T
    ...
    def eval_KL_expansion_trajectories_old(self, weights, nodes, eig, vect, M, traj_new,xi_traj):
        """
        This function computes the eigenfunctions from the result
        of the Nystrom method.
        ...
        M : int
            trunc number
        traj_new : (n_doe+n_mc) x n array
            n : coord nb of points 
            n_mc : nb of new points 
        xi_traj : array M x n_mc
            random vector associated to a traj
        ...
        """
        if self.normalize == True:
                traj_new = (traj_new- self.X_mean) / self.X_std
        n_doe, n_features = self.X.shape
        n_mc,n=traj_new.shape
        eig_t=eig[:M].reshape(1,M) 
        # Covariance matrix
        n_traj=xi_traj.shape[1]
        H_x=np.zeros((n_traj,n_mc))
        phi=self.eval_eigenfunction_noncond(weights, nodes, eig, vect, traj_new, M)
        for i in np.arange(n_mc):
            c=np.sqrt(1./eig_t).dot(phi[:,i].reshape(M,1)*xi_traj)
            H_x[:,i]=c.real
    ...
    def test_K_noncond(self, nodes):
        ...
        R_new[ij[:, 0], ij[:, 1]] = r_new
        R_new[ij[:, 1], ij[:, 0]] = r_new
        K_tot=self.sigma2*R_new
        return K_tot
    

    
    def eval_mean_t(self, X,y_traj):
        """
        This function computes difference mu(x)-mu_tilde(x) for sampling with a non cnditioned GP
    ...
    def delta_t(self, X,phi_doe, eig, M):
        """
        This function computes difference mu(x)-mu_tilde(x) for sampling with a non cnditioned GP
        ...
        eig_m=np.zeros((M,M))
        np.fill_diagonal(eig_m,np.sqrt(1./eig_t))
        phi_doe=eig_m.dot(phi_doe)
        phi_doe=phi_doe.real
        n_eval=X.shape[0]
        # Initialize output
        n_samples, n_features = self.X.shape
        n_doe,n_targets=self.y.shape
        ...
        r = self.corr(self.theta_, dx).reshape(n_eval, n_samples)
         ##########
        delta_t =np.zeros((n_eval,M))
        y_mean=phi_doe.mean(axis=1)
        y_std=phi_doe.std(axis=1)
        if self.normalize == True:
                phi_doe = (phi_doe- y_mean.reshape(M,1)) / y_std.reshape(M,1)
        Yt = linalg.solve_triangular(self.C_, phi_doe.T, lower=True)
        beta_t = linalg.solve_triangular(self.G_, np.dot(self.Q_.T, Yt))
        rho = Yt - np.dot(self.Ft_, beta_t)
        gamma_t = linalg.solve_triangular(self.C_.T, rho)
        y_ = np.dot(f, beta_t) + np.dot(r, gamma_t)
        delta_t = y_mean + y_std * y_
        
        # delta_t : L : n_mc et C : M
        return delta_t
