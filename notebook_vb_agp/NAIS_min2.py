...
def Boucle_k_d(k,rho,S,f,N,phi,g,population_X,population_Y,fraction,f_type,gp_g):
...
def NAIS_m(rho,S,f,N,gp_g,iter_max=60,f_type=True): #lancer ça pour l'avoir 1 fois
    #step 1
    k=1
    #step 2a
    if f_type : 
    ...
    while gamma > S and k < iter_max and alpha > eps_alpha : 
        k=k+1
        alpha=gamma
        Boucle=Boucle_k_d(k,rho,S,f,N,phi,g,population_X,population_Y,fraction,f_type,gp_g)
        gamma=Boucle[0]
...
#distribution_nonlin_osci_low_IS=stats.multivariate_normal(mean=[1.,0.1,1.,0.5,1.,0.6], cov=np.diag([0.01,0.01,0.0025,0.0025,0.04,0.1**2]))
...
#    Pf, f_aux, var_P, population_X =NAIS(rho=10,S=0.,f=distribution_nonlin_osci_low_IS,N=10000,phi=nonlin_osci_mc,iter_max=50)